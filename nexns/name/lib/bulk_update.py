def bulk_update(request, original_set, data, serializer_class, on_save_fn=None):
    ...
    for d in data:
        if d["id"] > 0:
            serializer = serializer_class(original_set.get(id=d["id"]), data=d, context={'request': request})
            serializer.is_valid(raise_exception=True)
            need_updated_set.append((serializer, d))
            continue

        del d["id"]
        serializer = serializer_class(data=d, context={'request': request})
        serializer.is_valid(raise_exception=True)
